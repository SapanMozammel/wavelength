	...
	<DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
		<DialogPrimitive.Portal>
			...
			<DialogPrimitive.Content
				className={cn(
					'bg-surface dark:bg-surface-dark border-border-subtle dark:border-border-subtle-dark rounded-panel fixed top-1/2 left-1/2 z-50 flex max-h-[85dvh] w-[calc(100%-2rem)] max-w-md -translate-1/2 flex-col gap-4 border px-3 py-5 shadow-lg shadow-black/10 dark:shadow-black/40',
					className
				)}
			>
				...
				{description !== undefined && <DialogPrimitive.Description className='text-ink-muted dark:text-ink-muted-dark -mt-2 px-2 text-sm text-pretty'>{description}</DialogPrimitive.Description>}
				<div className='min-h-0 flex-1 overflow-y-auto px-2'>{children}</div>
				{footer !== undefined && <div className='flex shrink-0 justify-end gap-2 px-2'>{footer}</div>}
			</DialogPrimitive.Content>
		</DialogPrimitive.Portal>
